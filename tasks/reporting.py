...
def research_source(source, requests_timeout):
    ...
    try:
        ...
        if source["method"]=="api":
            ...
            items = [item.replace("http:", "https:") for item in items] if source.get("enforce_https_img_url", False) else items
...
def get_car_talk_credit(bucket_path):
    ...
    try:
        ...
        logging.warning(f"Exception in get_car_talk_credit: {credit}") 
        return []
