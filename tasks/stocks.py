...
def research_stock_history(ticker):
    ...
    RETURNS
    Pandas DataFrame with previous month's stock prices
    """
    ...
    if "shortName" in stock_info:
        ...
        stock_name = ticker
    if len(stock_name) < 4:
        # If name is blank or unexpectedly short, use ticker
        stock_name = ticker

    # Get price series
    # By default we get the last quarter, the max we may need
    stock_df = (
        stock.history(period="3mo")
        .reset_index()
        .assign(
    ...
    return stock_df.set_index(["date"]).drop(columns=["Close"])
...
def research_stock_histories(tickers):
    ...
    RETURNS
    Pandas DataFrame with previous month's prices, with each stock as a column and mon-day (str) as index
    """

    stocks_l = [research_stock_history(ticker) for ticker in tickers]
    stocks_df = pd.concat(stocks_l, axis=1)
    # Sort biggest ticker first
    stocks_df = stocks_df.loc[:, stocks_df.max().sort_values(ascending=False).index]
    return stocks_df
...
def plot_stocks(stocks_df, history="quarter", dev_mode=False):
    ...
    if history == "quarter":
        ...
        ticks = pd.Index(set(list(stocks_df.index[::30])[0:-1] + [stocks_df.index[-1]]))
    elif history == "month":
        stocks_df = stocks_df.tail(30)
        # Tick for every week, and ensure we include last day. Set de-dups if necessary
        ticks = pd.Index(set(list(stocks_df.index[::7])[0:-1] + [stocks_df.index[-1]]))
    elif history == "week":
    ...
    sns.despine()
    plt.tight_layout()
    # Max of biggest ticker
    plot_max_y = stocks_df.iloc[:, 0].max()
    # Max of biggest ticker + 20%
    _ = plt.ylim(0, 1.2 * plot_max_y)
    ...
    if dev_mode:
        plt.savefig(
            f"stocks_{'_'.join(stocks_df.columns)}.png",
            format="png",
            bbox_inches="tight",
        )
...
def get_stocks_plot(tickers, section_frequency="monthly", dev_mode=False):
    ...
    RETURNS
    Image for a single plot of tickers as base64 (string)
    """
    ...
    if section_frequency == "monthly":
        ...
        logging.warning(
            f"Unexpected section_frequency in get_stocks_plot(): {tickers}, {section_frequency}"
        )
        return None
