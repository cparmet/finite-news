...
def get_nws_forecast(nws_config):
    ...
    try:
        ...
        if not daytime_forecasts:
            # No daytime forecasts found
            logging.warning(
                f"No NWS forecast added because no non-night/overnight period available. Config: {nws_config}. Response from NWS: {r.json()}."
            )
            print("error")
            return None
        # Get the daytime forecast that's coming first
        result = daytime_forecasts[0]
        ...
        # Change from Title Case to Sentence case
        forecast["short"] = forecast["short"].capitalize()
...
def get_ca_forecast(forecast_config):
    ...
    RETURNS
    Dictionary with attributes of the forecast retrieved, or None if there was a problem.
    """

    try:
        ec_en = ECWeather(coordinates=(forecast_config["lat"], forecast_config["lon"]))
        asyncio.run(ec_en.update())
        forecast_short, forecast_detailed = ec_en.daily_forecasts[0][
            "text_summary"
        ].split(".", maxsplit=1)
...
def get_gws_forecast(forecast_config):
    ...
    try:
        forecast_config["url"] = (
            f"https://opendata.dwd.de/weather/text_forecasts/html//{forecast_config['forecast_file']}"
        )
        ...
        logging.warning(
            f"German Weather Service forecast error: {str(type(e))}, {str(e)}, {forecast_config}"
        )
        return None
...
def get_forecast(forecast_config):
    ...
    if forecast_config["source"] == "env_canada":
        ...
        logging.warning(
            f"Unexpected forecast source. No forecast added. {forecast_config}"
        )
        return None
